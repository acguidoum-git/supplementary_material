...
HO_mom <- function(x0,gt,k=NULL,Alpha=1,Beta=1,Gamma=1,H=0.75,p=1,t)
    ...
    Gt  <- Deriv::Deriv(gt,"t",nderiv = k)}
    G_t <- function(t) eval(Gt)
    A   <- function(t) 2^(-0.5*p)*((Gamma*G_t(t)^(Alpha))/1i)^p * t^(H*p)
    B   <- function(t) 0.5*sqrt(2)*1i *((G_t(0)^(-Alpha)*x0+Beta*t )/Gamma)*t^(-H)            
    if (ttutils::isInteger(p)){
          Herm     <- EQL::hermite(x=B(t),n=p,prob=FALSE)}else{
    ...
    result <- Re(A(t)*Herm)
    if (length(t0)==1 & t0==0) {result <- c(x0^p,result)} else { result <- result}
