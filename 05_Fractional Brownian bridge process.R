...
##################################################################
##################################################################
###            Fractional Brownian bridge process             ####
##################################################################
##################################################################
##################################################################
library(future)
...
labels_expr <- sapply(sort(unique(df_Ex_long_55$p)), function(x) {
  if(x %% 1 == 0){
    deparse(bquote(M[.(x)](t)))
  } else {
    deparse(bquote(M[unique(df_Ex_long_55$p)](t)))
  }
})
